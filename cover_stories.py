# Creative addition: I added an extra sentence that automatically choosekks "a" or "an" before a user-provided noun based on its first letter.

print("Please enter the following:\n")

adjective = input("adjective: ")
animal = input("animal: ")
verb1 = input("verb: ")
exclamation = input("exclamation: ")
verb2 = input("verb: ")
verb3 = input("verb: ")

# Creative extra input
noun = input("noun: ")

# Capitalize exclamation
exclamation = exclamation.capitalize()

# Automatically choose "a" or "an"
article = "an" if noun[0].lower() in "aeiou" else "a"

print("\nYour story is:\n")

print(f"The other day, I was really in trouble. It all started when I saw a very")
print(f"{adjective} {animal} {verb1} down the hallway. \"{exclamation}!\" I yelled. But all")
print(f"I could think to do was to {verb2} over and over. Miraculously,")
print(f"that caused it to stop, but not before it tried to {verb3}")
print(f"right in front of my family. Later, I realized it was actually {article} {noun}.")
